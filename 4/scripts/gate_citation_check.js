...
//   (b) it is a bare filename that exists under `scripts/` or `4/scripts/`.
//
...
//     4/scripts/seat_arg_check.js       explicit 4/
...
//   (a) it lives under `4/scripts/`, so it is 4/-only by construction, or
...
// 4/scripts/seat_arg_check.js prints the 161 call sites it read (HARD-WON-LESSONS §1b). It also
...
for (const entry of chain.split("&&").map((x) => x.trim())) {
  ...
  if (!m) continue;
  const script = m[1];
  if (script.startsWith("4/scripts/") || /--tree=4(\s|$)/.test(entry)) coversFour.add(script);
}

/* ================= Resolving a cited name to a real file ================= */
// A file under 4/scripts/ covers 4/ by construction, whether or not it is in the chain — it can
...
function resolveCitation(explicitPrefix, base) {
  ...
  const four = `4/scripts/${base}`;
  const rootDirect = `scripts/${base}`;
  if (explicitPrefix === "4/scripts/") return candidates.has(four) ? four : null;
  ...
  // (4/scripts/lib_twin_check.js is what keeps that true).
...
function citationCoversFour(rel) {
  if (rel.startsWith("4/scripts/")) return true;
  return coversFour.has(rel);
}

/* ================= The scan ================= */
// Matches an optional `scripts/` or `4/scripts/` (with optional subdirectory) followed by a
...
for (const file of scanTargets) {
  ...
  lines.forEach((line, i) => {
    ...
    while ((m = CITATION_RE.exec(line))) {
      ...
      const base = m[3];
      const explicitPrefix = m[0].startsWith("4/scripts/")
        ? "4/scripts/"
...
if (universe.size === 0) {
  console.error(`FAIL: the script universe is EMPTY — nothing under scripts/ or 4/scripts/ was found, so every citation below would have been silently discarded as "not a gate name". The scanner is broken, not the tree.`);
  process.exit(1);
}
if (scanned === 0) {
