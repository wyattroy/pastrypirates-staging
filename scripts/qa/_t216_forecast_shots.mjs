 ...
 *
 * Sibling of _t216_sanctuary_shots.mjs, same shape, different sentence. The corrected FORECAST
 * sentence has to be READ, on both surfaces it reaches:
 ...
 * Both are scrolled to the Wind paragraph rather than photographed from the top, because a
 ...
 *
 * Ports differ from the sanctuary script's on purpose — another session is live on this machine and
 * two rigs on one debug port is a probe measuring somebody else's browser.
 */
...
launch(DBG, "/tmp/chrome-t216fcshots");
...
await sleep(900);
console.log("MODAL sentence:", await C.ev(`(()=>{const s=document.querySelector('[data-engine-rule="forecast"]');
  if(!s) return 'MISSING from the modal';
  s.scrollIntoView({block:'center'});
  return JSON.stringify(s.textContent.trim());})()`));
await sleep(600);
await shot("t216-modal-forecast-phone.png");

// 2 — the public page, scrolled to the same sentence
await go(`${origin}/rules.html`, 390, 844, true);
console.log("PAGE sentence:", await C.ev(`(()=>{const s=document.querySelector('[data-engine-rule="forecast"]');
  if(!s) return 'MISSING from rules.html';
  s.scrollIntoView({block:'center'});
  return JSON.stringify(s.textContent.trim());})()`));
await sleep(600);
await shot("t216-rules-forecast-phone.png");
