#!/usr/bin/env node
/* WHERE A CAPTAIN MAY SAIL THIS TURN IS ONE ANSWER — THE SQUARES THE CHOOSER IS OFFERED AND THE FRAME ITS OWN SCREEN PUTS ROUND THEM.
   ...
   the sail search WITH the trade winds' rim; the camera frame for that same turn came from Game.reachableFrom — the same search
   WITHOUT the rim — under a comment saying the two "agree by construction". Measured on 40 seeded boards: from 3,430 of 4,432
   legal sea squares the chooser was offered rim squares the frame left out (18,267 squares), and on 3,296 the framed rectangle
   itself was smaller. Now Game.sailChoices (engine) is the one answer; reachable(), camFrameTurn and Game.fleeSquares all ask it.
   ⚠ ITEM 18 HAD A SECOND HALF AND WYATT OVERRULED IT, 2026-09-17: "we cannot see other players sail squares (bots or humans) so
   ALL other players turns should be zoomed in on their boat for maximum immersion." A WATCHING screen no longer frames these
   squares at all — it frames that captain's boat, and scripts/qa/watching_camera_one_rule_check.mjs is what holds THAT. What is
   left here, and what still matters, is the screen BEING ASKED: the turn reaches it before its own gold squares are drawn, so its
   first frame is built from this same engine call.
   RULES (each run against deliberately broken copies below, built in memory from the real source):
     ...
     2. THE CHOOSER'S SCREEN ASKS IT — reachable() and camFrameTurn's not-yet-drawn frame both call sailChoices, and no screen file
        (anything outside src/engine) runs a sail search of its own: no sailStates(, sailSearch( or reachableFrom( call.
     3. POSED BOARDS — reachable() and camFrameTurn() compiled from the source text (real or broken) and Game.sailChoices /
        fleeSquares compiled onto a real Game: for every legal sea square on seeded boards, the frame the chooser's own screen asks
        for BEFORE its squares are drawn is exactly the squares it is about to be offered plus its own square; with those squares
        drawn it frames the same set; sailChoices is the rim-allowed search square for square; the flee squares equal it.
...
function rules(files) {
  ...
  const reach = fnBody(S[FLOW], "export function reachable("), cam = fnBody(S[STAGE], "function camFrameTurn(");
  ...
  // 2. THE CHOOSER'S SCREEN ASKS IT
  {
    ...
    rule([
      ...
      [!!cam && /\bg\.sailChoices\(\s*who\s*\)/.test(cam), "camFrameTurn (the chooser's frame before its squares are drawn) does not ask Game.sailChoices"],
  ...
  // 3. POSED BOARDS
  {
    ...
    rule(res.err ? [[false, res.err]] : [
      ...
      [res.early === 0, `on ${res.early} of ${res.poses} posed squares the chooser's frame before its squares are drawn is not the squares it is offered (first: ${res.first})`],
      [res.own === 0, `on ${res.own} of ${res.poses} posed squares the chooser's frame with its squares drawn is not those squares`],
...
function posed(eng, reachSrc, camSrc) {
  ...
  if (!reachSrc || !camSrc) throw new Error("reachable() or camFrameTurn() could not be found");
  const reachable = new Function("appState", `${reachSrc.replace(/^export\s+/, "")}; return reachable;`);
  /* camToCell is injected as a throw: on the screen BEING ASKED the frame must never take the watcher's
     boat branch, and if it ever does this rule says so instead of quietly measuring the wrong function. */
  const boat = () => { throw new Error("camFrameTurn framed the BOAT on the chooser's own screen"); };
  const camFrameTurn = new Function("S", "appState", "document", "camFitCells", "camToCell", "SEAT_ZOOM",
    `${camSrc}; return camFrameTurn;`);
  const key = c => c[0] + "," + c[1], same = (a, b) => a.size === b.size && [...a].every(k => b.has(k));
  const r = { poses: 0, rimPoses: 0, early: 0, own: 0, rim: 0, flee: 0, first: "" };
  for (const seed of SEEDS) {
    ...
    for (let x = 0; x < n; x++) for (let y = 0; y < n; y++) {
      ...
      let framed = null;
      camFrameTurn({}, appState, { querySelectorAll: () => [], querySelector: () => null }, cells => { framed = new Set(cells.map(key)); }, boat, 1.9)(0, null, true);
      if (!framed || !same(framed, chooser)) { r.early++; if (!r.first) r.first = `seed ${seed} square ${key(c)}: offered ${chooser.size}, framed ${framed ? framed.size : "nothing"}`; }
      const drawn = [...gold, c].map(q => ({ dataset: { gx: q[0], gy: q[1] } }));
      let own = null;
      camFrameTurn({}, appState, { querySelectorAll: sel => sel === ".sailCell" ? drawn : [], querySelector: () => null }, cells => { own = new Set(cells.map(key)); }, boat, 1.9)(0, null, true);
...
const NAMES = ["the one answer: Game.sailChoices is the sail search with the rim allowed, and nothing else builds that list (fleeSquares asks it)",
  "the chooser's screen asks it: reachable() and camFrameTurn call Game.sailChoices, and no screen runs a sail search of its own",
  "posed boards: the chooser's screen frames exactly the squares it is offered, before its gold squares are drawn and after, and the flee squares are the same answer"];
...
const MUTANTS = [
  [[2, 3], "camFrameTurn back on reachableFrom (the frame without the rim) — the fault this item removed",
...
console.log(fails || !proofOk ? `\nFAIL — ${fails} rule(s) broken${proofOk ? "" : ", and the red-proof did not hold"}`
  : `\nPASS — where a captain may sail is one engine answer, and the screen being asked frames exactly the squares it is offered; ${real.length} rules, ${MUTANTS.length} mutants, each red`);
