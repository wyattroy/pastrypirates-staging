# `scripts/qa/` — one-off reproductions, NOT the process

**The process is the sea trial: `node scripts/sea_trial.mjs`.** See `docs/QA-PROCESS.md`.
...
| deleted | because |
|---|---|
| `matrix.mjs` | `scripts/playtest_gate.mjs` already played whole voyages across modes and sizes, with a real mouse, universal checks and a vision judge. Written 2026-08-21 to Wyatt's own words. |
| `checks.mjs` | `scripts/lib/checks.mjs` already held UNIVERSAL structural rules by ROLE. Its own header says it is "the opposite of the piecemeal gate Wyatt (rightly) rejected: add no rule per bug" — and the deleted file was exactly ten per-bug rules. |
| `lib/advance.mjs` | `scripts/lib/player.mjs` already knew how to play the game fully: real mouse, on-screen gate, coverage-first choices, dead-button detection. |
...
They live in `scripts/` and run in `npm test`, which is the difference:
