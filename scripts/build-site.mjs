...
const EXCLUDE_FILES = new Set([
  ...
  /* wrangler.toml was DELETED from the repo on 2026-09-15 (PRD-CLOUDFLARE-CUTOVER P2; the reasons
     are in docs/GIT-AND-DEPLOY.md §5). Named here only so a re-added one is never served. */
  "wrangler.toml",
  "README.md",
...
/* THE ADDRESSES ARE DERIVED FROM THE PUBLISH SET (PRD-CLOUDFLARE-CUTOVER P5). Cloudflare redirects
   `/about.html` to `/about` and `/classic/index.html` to `/classic/`, so a player's page arrives at
   an extension-less address that `/*.html` never matches. So every published page gets its own
   rule, and a page added tomorrow is covered the moment it is committed; `/` is just index.html's
   entry. The same for every game tree's `src/`: `/src/*` is anchored at the root, so
   `/classic/src/` matched nothing. Each address is written ONCE, because Cloudflare JOINS a
   repeated header's values with a comma. (Cloudflare's documented default for an unmatched asset
   is this same value today; these rules make it this repo's property, not an assumption about
   theirs.) `scripts/qa/site_build_check.mjs` case 7 derives the same set independently. */
const REVALIDATE = "  Cache-Control: public, max-age=0, must-revalidate\n";
const pageAddresses = [...new Set(publish.filter((p) => p.endsWith(".html"))
  .map((p) => "/" + p.replace(/(^|\/)index\.html$/, "$1").replace(/\.html$/, "")))].sort();
const moduleAddresses = [...new Set(publish.map((p) => {
  const segs = p.split("/");
  const at = segs.indexOf("src");
  return at > -1 && at < segs.length - 1 ? "/" + segs.slice(0, at + 1).join("/") + "/*" : null;
}).filter(Boolean))].sort();
const revalidateRules = [...pageAddresses, ...moduleAddresses].map((a) => `${a}\n${REVALIDATE}`).join("");
