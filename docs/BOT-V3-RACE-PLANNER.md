...
*(⛔ 2026-09-18: `scripts/measure_race_spread.mjs` is RETIRED to `scripts/tool_archive/` — the two
numbers above are a RECORD of what it measured, not a command you can re-run. It fitted them with
every seat on `planTurnClassic`, deliberately, so that the dynamics being described could not be
generated by the constant being fitted; that control brain was deleted from the engine on Wyatt's
ruling that the old brain is done. **The instrument's own header says to re-run it if the movement,
dock, battle or bake rules ever change — and they have.** Re-fitting them against the shipping brain
would calibrate the curve against dynamics the curve itself produces, which is a measurement design
question for Wyatt, not a re-point. The constants stand at `src/engine/index.js:67-74` until he
rules otherwise.)*
<!-- doc-check: allow scripts/measure_race_spread.mjs -->
<!-- doc-check: allow scripts/bot_ladder3.js -->
...
*(⛔ 2026-09-18: `scripts/bot_ladder3.js` is RETIRED to `scripts/tool_archive/` — it imported `../3/`,
deleted at the cutover, and it dispatched between `planTurnV3` and `planTurnClassic`, a pairing this
engine no longer holds. The table below is the RECORD of what it measured in its day; the numbers
stand, the command does not. Today's instrument is `scripts/bot_ladder4.js`, on a time axis.)*
